...
- Tray menu has exactly 3 items: 먹이 주기 / 사진 찍기 / 퇴근 (spec section 1 & clarification). No separate "종료" item.
...
### Task 14: Camera photo window

**Files:**
- Create: `electron/cameraWindow.js`
- Create: `renderer/camera.html`
- Create: `renderer/camera.js`
- Modify: `electron/main.js`
- Modify: `electron/tray.js` (no signature change — the `onPhoto` callback is already wired, just now does something)

**Interfaces:**
- Consumes: `IPC.OPEN_CAMERA` (unused directly — camera window is a fully separate `BrowserWindow`, not the overlay, so it doesn't need overlay IPC).
- Produces: `openCameraWindow()` from `cameraWindow.js`, creating a normal (non-transparent, framed) `BrowserWindow` loading `renderer/camera.html`.

- [ ] **Step 1: Implement cameraWindow.js**
...
- [ ] **Step 2: Implement the camera renderer page**

`renderer/camera.html`:

```html
<!DOCTYPE html>
<html>
<head><meta charset="utf-8"><title>비둘기와 사진 찍기</title></head>
<body style="margin:0;text-align:center;font-family:sans-serif;">
  <video id="webcam" width="480" height="360" autoplay></video><br/>
  <button id="capture-btn">촬영</button>
  <p id="status"></p>
  <canvas id="output" width="480" height="360" style="display:none;"></canvas>
...
document.getElementById('capture-btn').addEventListener('click', async () => {
  ...
  const result = await window.pigeonBridge.send('save-photo', dataUrl);
  status.textContent = '사진을 저장했어요!';
});
```

- [ ] **Step 3: Handle save-photo and openCameraWindow in main.js**
...
Update the tray's `onPhoto` callback (from Task 7's `createTray` call) to actually open the window:

```js
createTray(
  () => overlayWin.webContents.send(IPC.FEED_TRIGGERED),
  () => openCameraWindow(),
  () => { /* commute-out, as in Task 7 */ }
);
```

- [ ] **Step 4: Switch preload's send to invoke for save-photo (needs a return value)**

Modify `renderer/preload.js` — add an `invoke` passthrough alongside the existing `send`/`on`:
...
Modify `renderer/camera.js` — change `window.pigeonBridge.send('save-photo', dataUrl)` to `window.pigeonBridge.invoke('save-photo', dataUrl)`.

- [ ] **Step 5: Manual verification**

Run: `npm start`. Click tray "사진 찍기" — a window opens showing webcam preview (grant camera permission if prompted). Click "촬영" — a save dialog appears; save it, then open the saved PNG and confirm it shows the webcam frame with a small colored square overlay in the corner. Deny camera permission (or test on a machine without a webcam) and confirm the window shows the Korean error message instead of crashing.

- [ ] **Step 6: Commit**

```bash
git add electron/cameraWindow.js renderer/camera.html renderer/camera.js electron/main.js renderer/preload.js
git commit -m "feat: camera photo window with webcam+pigeon composite capture"
```
